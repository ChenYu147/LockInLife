...
receive_data_info receive_data = {NULL, 0, 0};
...
static void on_write(ble_wechat_t *p_wcs, ble_evt_t * p_ble_evt)
{
				...
				p_evt_write->len <= BLE_WECHAT_MAX_DATA_LEN) 
		{
			if (g_rcv_data.len == 0) 
			{
				if(p_evt_write->data[0]==0x7e)
				{
						g_rcv_data.len=p_evt_write->data[1];
						g_rcv_data.offset = 0;
//						g_rcv_data.data = (uint8_t *)malloc(g_rcv_data.len);						
			...
			memcpy(g_rcv_data_data+g_rcv_data.offset, p_evt_write->data, chunk_size);
			g_rcv_data.offset += chunk_size;
			if (g_rcv_data.len <= g_rcv_data.offset) 
			{
				receive_data.data = & g_rcv_data_data[2];	//接收到的数据头两位为 7E+长度
				receive_data.len=g_rcv_data.len;
				receive_data.flag=1;				
				g_rcv_data.len = 0;
				g_rcv_data.offset = 0;				
			}	
...
/*
ble_wechat_indicate_data 函数调用 ble_wechat_indicate_data_chunk 函数进行分帧发送
（如果需要分帧发送的话）。发送完一帧之后会有一个确认事件	BLE_GATTS_EVT_HVC
*/
						break;	
...
static uint32_t ble_wechat_add_read_char(ble_wechat_t *p_wechat)
{
    ...
    attr_char_value.init_offs    = 0;
    attr_char_value.max_len      = BLE_WECHAT_MAX_DATA_LEN;
    attr_char_value.p_value      = m_addl_adv_manuf_data;			//MAC地址
